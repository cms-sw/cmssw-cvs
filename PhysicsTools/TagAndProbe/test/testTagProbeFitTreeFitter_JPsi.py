...
###  The input file has been produced with PhysicsTools/TagAndProbe/test/testTagProbeFitTreeProducer_JPsi.py
###  The original October Excercise cfg is at MuonAnalysis/TagAndProbe/test/jpsi/fit.py
...
RunFit = cms.EDAnalyzer("TagProbeEDMAnalysis",  
      ...
      Mode = cms.untracked.string("ReadNew"), ## <<< Instead of "Read"

      ReadFromFiles     = cms.untracked.vstring("testTagProbeFitTreeProducer_JPsi.root"), ## << Always the same file
      ReadFromDirectory = cms.untracked.string("TO_BE_FILLED"), ## << This changes from fit to fit
...
process.fitGlbFromTk = RunFit.clone(
    ReadFromDirectory = 'fitGlbFromTk',  ## << set Directory instead of File
    FitFileName       = 'fit_result_GlbFromTk.root',
)

process.fitTkFromSta = RunFit.clone(
    ReadFromDirectory = 'fitTkFromSta',  ## << set Directory instead of File
...
process.fitHltFromGlb = RunFit.clone(
    ReadFromDirectory = 'fitHltFromGlb',  ## << set Directory instead of File
...
process.TFileService = cms.Service("TFileService", fileName = cms.string("testTagProbeFitTreeFitter_JPsi.root"))
