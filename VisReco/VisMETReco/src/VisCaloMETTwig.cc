      ...
      m_text ("no info")
{
    ...
    edm::TypeID metCollID (typeid (CaloMETCollection));
...
VisCaloMETTwig::update (IgTextRep *rep)
{
	...
	try 
	{
	    for (std::vector<edm::Handle<CaloMETCollection> >::iterator i = m_handles.begin (), iEnd = m_handles.end (); 
		 i != iEnd; ++i) 
	    {
		const CaloMETCollection *met = (*i).product ();
