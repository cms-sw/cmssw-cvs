...
namespace 
{
    ...
    {
	return new VisCaloMETTwig (state, parent, "Calo MET (" + name + ")", 
...
VisCaloMETTwig::VisCaloMETTwig (IgState *state, IgTwig *parent,
				const std::string &name /* = "" */,
				const std::string &friendlyName /* = "" */,
      ...
      m_text ("no info")
{
    ...
    edm::TypeID metCollID (typeid (reco::CaloMETCollection));
    tfService->registerTwig (metCollID.friendlyClassName (), &createThisTwig);
}

void
VisCaloMETTwig::onNewEvent (const edm::Event &event,
...
void
VisCaloMETTwig::update (IgTextRep *rep)
{
    // Get debugging dump.
	...
	try 
	{
	    for (std::vector<edm::Handle<reco::CaloMETCollection> >::iterator i = m_handles.begin (), iEnd = m_handles.end (); 
		 i != iEnd; ++i) 
	    {
		const reco::CaloMETCollection *met = (*i).product ();
		...
		text << "<TR align = right>"
		     ...
		     << "<TD>" << std::setw (5) << std::setprecision (3) << et << "</TD>"
...
void
VisCaloMETTwig::update (Ig3DRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
...
void
VisCaloMETTwig::update (IgLegoRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
...
void
VisCaloMETTwig::update (IgRPhiRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
...
void
VisCaloMETTwig::update (IgRZRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
