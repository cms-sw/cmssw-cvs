...
namespace 
{
    ...
    {
	return new VisGenMETTwig (state, parent, "Gen MET (" + name + ")", 
...
VisGenMETTwig::VisGenMETTwig (IgState *state, IgTwig *parent,
			      const std::string &name /* = "" */,
			      const std::string &friendlyName /* = "" */,
      ...
      m_text ("no info")
{
    ...
    edm::TypeID metCollID (typeid (reco::GenMETCollection));
    tfService->registerTwig (metCollID.friendlyClassName (), &createThisTwig);
}

void
VisGenMETTwig::onNewEvent (const edm::Event &event,
...
void
VisGenMETTwig::update (IgTextRep *rep)
{
    // Get debugging dump.
	...
	try 
	{
	    for (std::vector<edm::Handle<reco::GenMETCollection> >::iterator i = m_handles.begin (), iEnd = m_handles.end (); 
		 i != iEnd; ++i) 
	    {
		const reco::GenMETCollection *met = (*i).product ();
		const reco::GenMET caloMET = met->front ();
...
void
VisGenMETTwig::update (Ig3DRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
...
void
VisGenMETTwig::update (IgLegoRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
...
void
VisGenMETTwig::update (IgRPhiRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
...
void
VisGenMETTwig::update (IgRZRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
