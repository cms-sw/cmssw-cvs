      ...
      m_text ("no info")
{
    ...
    edm::TypeID metCollID (typeid (GenMETCollection));
...
VisGenMETTwig::update (IgTextRep *rep)
{
	...
	try 
	{
	    for (std::vector<edm::Handle<GenMETCollection> >::iterator i = m_handles.begin (), iEnd = m_handles.end (); 
		 i != iEnd; ++i) 
	    {
		const GenMETCollection *met = (*i).product ();
		const GenMET caloMET = met->front ();
