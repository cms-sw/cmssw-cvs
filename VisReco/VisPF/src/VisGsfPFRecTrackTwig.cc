...
namespace 
{
    ...
    {
	return new VisGsfPFRecTrackTwig(state, parent, name, friendlyName, modLabel, instanceName, processName);
    }	
}

VisGsfPFRecTrackTwig::VisGsfPFRecTrackTwig(IgState *state, IgTwig *parent, 
					   const std::string &name,
					   const std::string &friendlyName, 
...
void VisGsfPFRecTrackTwig::onNewEvent(const edm::Event &event,
				      const edm::EventSetup &eventSetup)
{
    ...
    std::vector<edm::Handle<reco::GsfPFRecTrackCollection> > trackCollections;
    ...
    {
	for (std::vector<edm::Handle<reco::GsfPFRecTrackCollection> >::iterator i = 
		 trackCollections.begin(), iEnd = trackCollections.end(); 
	     i != iEnd; ++i) 
	{
	    const reco::GsfPFRecTrackCollection& c = *(*i);

	    for (std::vector<reco::GsfPFRecTrack>::const_iterator ipft = c.begin(), ipftEnd = c.end();
		 ipft != ipftEnd; ++ipft)
	    {
...
void  VisGsfPFRecTrackTwig::update(Ig3DRep *rep)
{
...
void  VisGsfPFRecTrackTwig::update(IgLegoRep *rep)
{
     VisQueuedTwig::update(rep);
}

void  VisGsfPFRecTrackTwig::update(IgTextRep *rep)
{
    VisQueuedTwig::update(rep);
    ...
    text << m_name << " from ";
    text << m_text << "<br>";
    text << "Total: " << m_tracks.size () << " GsfPFRecTracks.<br>";
	...
	if ( ! m_tracks.empty() )
	{
	    ...
	    for ( std::vector<reco::GsfPFRecTrack>::iterator iT = m_tracks.begin();
		  iT != m_tracks.end(); ++iT )
	    {
...
void  VisGsfPFRecTrackTwig::update(IgRPhiRep *rep)
{
    VisQueuedTwig::update (rep);
...
void  VisGsfPFRecTrackTwig::update(IgRZRep *rep)
{
    VisQueuedTwig::update(rep);
