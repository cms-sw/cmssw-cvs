...
namespace 
{
    ...
    {
	return new VisCSCComparatorDigiTwig (state, parent, "Comparator digis (" + name + ")", 
...
VisCSCComparatorDigiTwig::VisCSCComparatorDigiTwig (IgState *state, IgTwig *parent,
						    const std::string &name /* = "" */,
						    const std::string &friendlyName /* = "" */,
      ...
      m_processName (processName)
{
    ...
    edm::TypeID caloJetsID (typeid (CSCComparatorDigiCollection));
    tfService->registerTwig (caloJetsID.friendlyClassName (), &createThisTwig);
}

void
VisCSCComparatorDigiTwig::onNewEvent (const edm::Event &event,
				      const edm::EventSetup &eventSetup)
{
    ...
    std::vector<edm::Handle<CSCComparatorDigiCollection> > digiCollections;
    ...
    {
	std::vector<edm::Handle<CSCComparatorDigiCollection> >::iterator i;
	std::vector<edm::Handle<CSCComparatorDigiCollection> >::iterator iEnd;
	for (i = digiCollections.begin (), iEnd = digiCollections.end (); i != iEnd; ++i) 
	{
	    const CSCComparatorDigiCollection& c = *(*i);
    
	    // Loop over the DetUnits with digis
	    CSCComparatorDigiCollection::DigiRangeIterator detUnitIt;
	    CSCComparatorDigiCollection::DigiRangeIterator detUnitItEnd;
		...
		for (CSCComparatorDigiCollection::const_iterator digiIt = range.first;
		     ...
		     ++digiIt)
		{
		    std::pair< CSCDetId, CSCComparatorDigi > p;
		    p.first = id;
		    p.second = (*digiIt);
...
void
VisCSCComparatorDigiTwig::update (IgTextRep *rep)
{
    // Get debugging dump.
	...
	try 
	{
	    for (std::vector<std::pair<CSCDetId, CSCComparatorDigi> >::const_iterator i = m_digis.begin (), iEnd = m_digis.end (); i != iEnd; ++i) 
	    {
		...
		text << "<TR align = right>"
		     ...
		     << "<TD>" << std::setw (6) << std::setprecision (3) << (*i).second.getComparator () << "</TD>"
		     << "<TD>" << std::setw (5) << std::setprecision (3) << (*i).second.getTimeBin () << "</TD></TR>";
	    }
	}
	catch (cms::Exception& e)
	{
...
void
VisCSCComparatorDigiTwig::update (Ig3DRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    //  
	...
	//    const int SignalThreshold = 701; //ADC counts
	//
	SbColor grey35;
	...
	try 
	{
	    ...
	    matl->diffuseColor.setValue (0.1, 1.0, 0.4); // yellow
	    matl->emissiveColor.setValue(0.1, 1.0, 0.4);	  
	    matl->shininess = 1.;
	    //  
	    ...
	    //
	    for (std::vector<std::pair<CSCDetId, CSCComparatorDigi> >::const_iterator 
		...
		Surface::GlobalPoint pos = (m_pDD->idToDet (id))->surface ().toGlobal
...
void
VisCSCComparatorDigiTwig::update (IgLegoRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
	...
	for (std::vector<std::pair<CSCDetId, CSCComparatorDigi> >::const_iterator i = m_digis.begin (), iEnd = m_digis.end (); 
	     i != iEnd; ++i) 
	{
	    Surface::GlobalPoint pos = (m_pDD->idToDet ((*i).first))->surface ().position ();
// 	    double eta = pos.eta ();
...
// 	    // int strip = (*i).second.getComparator ();
...
void
VisCSCComparatorDigiTwig::update (IgRPhiRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
...
void
VisCSCComparatorDigiTwig::update (IgRZRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
