...
void VisCuTkSlWindow:: setTracker(VisCuCmsTracker *tr)
{
   ...
   ASSERT (tracker);
   std::cout << "tracker components " << tracker->components()<< std::endl;
}
...
void VisCuTkSlWindow:: drawpart(VisCuTkState* object)
{
  ...
  if(dynamic_cast<VisCuTkLayer*>(object)){
    ...
    lay->setVisible(yn);
    std::cout << "Layer "<< lay->getId()<< " of "<< lay->getOwner()->getName()<< " selected" << std::endl;
  ...
  if(dynamic_cast<VisCuTkSubLayer*>(object)){
    ...
    subly->setVisible(yn);
    std::cout << "Ring "<< subly->getId()<< " layer "<<subly->getOwner()->getId()<<" of "<< subly->getOwner()->getOwner()->getName()<< " selected" << std::endl;
  ...
  if(dynamic_cast<VisCuTkModule*>(object)){
    ...
    std::cout << "Module " << module->getId() << " ring "<<module->getOwner()->getId()<<" in " << module->getOwner()->getOwner()->getOwner()->getName() << " "<< module->getOwner()->getOwner()->getId()<< std::endl;
  }
  repaintAll();
}
