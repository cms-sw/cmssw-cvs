				 ...
				 const edm::EventSetup &eventSetup)
{
  ...
  if (! m_done)
  {
	...
	cout << "winTkMap updated "<<endl; 
      }
      if(winTkSl!=0){
	winTkSl->setTracker(tr);
	winTkSl->setMap(winTkMap->map);
	winTkSl->drawall();
	cout << "winTkSl update "<<endl; 
...

{
  ...
  if(crossedModulesOnly.value()){
    ...
    {
      cout << "no tracks type"<<trackProducerName.value()<<"/"<<trackLabelName<< " in event" << endl;
    }
    if(!notracks){
      int nt = 0;
