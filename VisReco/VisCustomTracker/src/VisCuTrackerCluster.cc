			       ...
			       const edm::EventSetup &eventSetup)
{
  ...
  if(stripclusterProducerName!="ignore")
  {
    ...
    if (! s_cluster_collections.empty ())
      nocluster=false;
    if(nocluster)
      cout << "no stripclusters type "<<stripclusterProducerName<<"/"<<stripclusterLabelName<< " in event" << endl;
    if(!nocluster)
    {
  ...
  if(pixelclusterProducerName!="ignore")
  {
    ...
    if (! p_cluster_collections.empty ())
      nocluster=false;
    if(nocluster)
      cout << "no pixelcluster type "<<pixelclusterProducerName<<"/"<<pixelclusterLabelName<< " in event" << endl;
    if(!nocluster)
    {
