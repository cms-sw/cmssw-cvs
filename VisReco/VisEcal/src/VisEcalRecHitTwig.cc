...
namespace 
{
	...
	IgTwig *eventTwig = 0;
	eventTwig = rootTwig->lookup ("/Objects/CMS Event and Detector/ECAL Event/Rec Hits");
	
	if (! eventTwig)
	    eventTwig = parent;

	return new VisEcalRecHitTwig (state, eventTwig, "[N/A] Ecal RecHits (" + name + ")", 
...
VisEcalRecHitTwig::VisEcalRecHitTwig (IgState *state, IgTwig *parent,
				      const std::string &name /* = "" */,
				      const std::string &friendlyName /* = "" */,
    ...
    : VisQueuedTwig (state, parent, name),
      ...
      m_binning (state, lat::CreateCallback (this, &VisEcalRecHitTwig::twigChanged)),
      m_scale (state, lat::CreateCallback (this, &VisEcalRecHitTwig::twigChanged)),
      m_cut (state, lat::CreateCallback (this, &VisEcalRecHitTwig::twigChanged)),
      m_escale (state, lat::CreateCallback (this, &VisEcalRecHitTwig::twigChanged)),
      m_annotation (state, lat::CreateCallback (this, &VisEcalRecHitTwig::twigChanged)),
      m_rMax (state, lat::CreateCallback (this, &VisEcalRecHitTwig::twigChanged)),
      m_text ("no info"),
      m_recHits (500),
      m_rgba (colour)
{
    ...
    edm::TypeID recHitCollID (typeid (EcalRecHitCollection));
    tfService->registerTwig (recHitCollID.friendlyClassName (), &createThisTwig);
}

void
VisEcalRecHitTwig::twigChanged (void)
{ IgRepSet::invalidate (this, SELF_MASK); }

void
VisEcalRecHitTwig::onNewEvent (const edm::Event &event,
			       const edm::EventSetup &eventSetup)
{
    ...
    std::vector<edm::Handle<EcalRecHitCollection> > rechits;
    try
    {
	QApplication::setOverrideCursor (Qt::waitCursor);
	if (IgQtAppStatusBarService *sbar = IgQtAppStatusBarService::get (state ()))
	    sbar->setMessage ("Retrieving an EcalRecHitCollection...");
	...
	if (IgQtAppStatusBarService *sbar = IgQtAppStatusBarService::get (state ()))
	    sbar->setMessage ("EcalRecHitCollection done.");
	QApplication::restoreOverrideCursor ();
    }
    catch (cms::Exception& e)
    {
	e.append (" from VisEcalRecHitTwig::onNewEvent ");
	e.append (this->name ());    
    ...
    if (! rechits.empty ())
    {	
	for (std::vector<edm::Handle<EcalRecHitCollection> >::iterator i = rechits.begin (), iEnd = rechits.end (); i != iEnd; ++i) 
	{
	    const EcalRecHitCollection& c = *(*i);
	    ...
	    for (std::vector<EcalRecHit>::const_iterator ci = c.begin (), ciEnd = c.end (); ci != ciEnd; ++ci)
	    {
...
void
VisEcalRecHitTwig::update (IgTextRep *rep)
{
    // Get debugging dump.
	...
	if (! m_recHits.empty ()) 
	{
	    ...
	    try 
	    {
		for (std::vector<EcalRecHit>::const_iterator i = m_recHits.begin (), iEnd = m_recHits.end (); i != iEnd; ++i) 
		{
		    float energy = (*i).energy ();
		    if (energy > m_cut.value ())
		    {
			const GlobalPoint& pos = esService->getCellPosition ((*i).detid ());		
			float eta = pos.eta ();
			float phi = pos.phi ();
...
void
VisEcalRecHitTwig::update (Ig3DRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
	...
	{
	    e.append (" from VisEcalRecHitTwig::update(Ig3DRep*) ");
	    e.append (this->name ());    
...
void
VisEcalRecHitTwig::update (IgLegoRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
    ...
    {
	e.append (" from VisEcalRecHitTwig::update(IgLegoRep *) ");
	e.append (this->name ());    
...
void
VisEcalRecHitTwig::update (IgRPhiRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
    ...
    {
	e.append (" from VisEcalRecHitTwig::update (IgRPhiRep*) ");
	e.append (this->name ());    
...
void
VisEcalRecHitTwig::update (IgRZRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
	...
	if (! m_recHits.empty ()) 
	{
		...
		if (energy > m_cut.value ())
		{
		    binNumber = esService->getBinNumber ((*i).detid (), nBinsR, nBinsZ, radiusR, radiusZ, barrelMaxEta, beamPipeTheta, endcapMaxTheta);
