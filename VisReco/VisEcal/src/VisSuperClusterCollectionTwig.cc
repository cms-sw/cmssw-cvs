...
namespace 
{
	...
	IgTwig *eventTwig = 0;
	eventTwig = rootTwig->lookup ("/Objects/CMS Event and Detector/ECAL Event/Super Clusters");
	
	if (! eventTwig)
	    eventTwig = parent;

	return new VisSuperClusterCollectionTwig (state, eventTwig, "[N/A] Super Cluster (" + name + ")", 
...
VisSuperClusterCollectionTwig::VisSuperClusterCollectionTwig (IgState *state, IgTwig *parent,
					  const std::string &name /* = "" */,
					  const std::string &friendlyName /* = "" */,
    ...
    : VisQueuedTwig (state, parent, name),
      ...
      m_binning (state, lat::CreateCallback (this, &VisSuperClusterCollectionTwig::twigChanged)),
      m_scale (state, lat::CreateCallback (this, &VisSuperClusterCollectionTwig::twigChanged)),
      m_cut (state, lat::CreateCallback (this, &VisSuperClusterCollectionTwig::twigChanged)),
      m_escale (state, lat::CreateCallback (this, &VisSuperClusterCollectionTwig::twigChanged)),
      m_annotation (state, lat::CreateCallback (this, &VisSuperClusterCollectionTwig::twigChanged)),
      m_rgba (colour)
{    
    ...
    edm::TypeID basicClusterID (typeid (reco::SuperClusterCollection));
    tfService->registerTwig (basicClusterID.friendlyClassName (), &createThisTwig);
}

void
VisSuperClusterCollectionTwig::twigChanged (void)
{ IgRepSet::invalidate (this, SELF_MASK); }

void
VisSuperClusterCollectionTwig::onNewEvent (const edm::Event &event,
					   const edm::EventSetup &eventSetup)
{
    ...
    std::vector<edm::Handle<reco::SuperClusterCollection> > clusters;

    try 
    {
	QApplication::setOverrideCursor (Qt::waitCursor);
	if (IgQtAppStatusBarService *sbar = IgQtAppStatusBarService::get (state ()))
	    sbar->setMessage ("Retrieving a SuperClusterCollection...");
	...
	if (IgQtAppStatusBarService *sbar = IgQtAppStatusBarService::get (state ()))
	    sbar->setMessage ("SuperClusterCollection done.");
	QApplication::restoreOverrideCursor ();
    }
    catch (cms::Exception& e)
    {
	e.append (" from VisSuperClusterCollectionTwig::onNewEvent ");
	e.append (this->name ());    
    ...
    if (! clusters.empty ()) 
    {
	std::vector<edm::Handle<reco::SuperClusterCollection> >::iterator i;
	std::vector<edm::Handle<reco::SuperClusterCollection> >::iterator iEnd;
	for (i = clusters.begin (), iEnd = clusters.end (); i != iEnd; ++i) 
	{
	    const reco::SuperClusterCollection& c = *(*i);
	    ...
	    for (reco::SuperClusterCollection::const_iterator j = c.begin (), jEnd = c.end (); j != jEnd; ++j) 
	    {
		m_clusters.push_back (*j);
		QString name = QString ("Super Cluster Et = %1 GeV")
			       .arg ((*j).energy (), 0, 'f', 2);
		...
		new VisSuperClusterTwig (state (), this, name.latin1 (), m_clusterRecHits);
	    }
	}
    }
...
void
VisSuperClusterCollectionTwig::update (IgTextRep *rep)
{
    // Get debugging dump.
	...
	if (! m_clusters.empty ()) 
	{
	    try 
	    {
		for (std::vector<reco::SuperCluster>::const_iterator i = m_clusters.begin (), iEnd = m_clusters.end (); i != iEnd; ++i) 
		{
			...
			text << "<TR align = right>"
			     ...
			     << "<TD>" << std::setw (5) << std::setprecision (3) << (*i).rawEnergy () << "</TD>"
			     << "<TD>" << std::setw (5) << std::setprecision (3) << (*i).preshowerEnergy () << "</TD>"
	    ...
	    {
		e.append (" from VisSuperClusterCollectionTwig::update(IgTextRep*) ");
		e.append (this->name ());    
...
void
VisSuperClusterCollectionTwig::update (Ig3DRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
	...
	{
	    e.append (" from VisSuperClusterCollectionTwig::update(Ig3DRep*) ");
	    e.append (this->name ());    
...
void
VisSuperClusterCollectionTwig::update (IgLegoRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
    ...
    {
	e.append (" from VisSuperClusterCollectionTwig::update(IgLegoRep *) ");
	e.append (this->name ());    
...
void
VisSuperClusterCollectionTwig::update (IgRPhiRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
    ...
    {
	e.append (" from VisSuperClusterCollectionTwig::update (IgRPhiRep*) ");
	e.append (this->name ());    
...
void
VisSuperClusterCollectionTwig::update (IgRZRep *rep)
{
    // Get debugging dump.
    VisQueuedTwig::update (rep);
    rep->clear ();
    ...
    {
	e.append (" from VisSuperClusterCollectionTwig::update(IgRZRep*) ");
	e.append (this->name ());
