...
#define INSERT_SUB_1(t1,a1)						\
  ...
  {									\
      ...
      clearDummy ();							\
									\
      QMenu	*subpopup = new QMenu (m_menu);		\
...
#define INSERT_SUB_2(t1,a1,t2,a2)					\
  ...
  {									\
      ...
      unsigned		pos = itemPosition (id);			\
      m_submenus.insert (m_submenus.begin () + subpos, submenu);	\
      m_menu->insertItem (a1, "submenu", subpopup, id, pos);	      	\
      return submenu;							\
  }
...
#define INSERT_SIMPLE_2(t1,a1,t2,a2)					\
  ...
  {									\
      ...
      clearDummy ();							\
									\
      unsigned pos = itemPosition (id);					\
      VERIFY (m_menu->insertItem (a1, "simple", id, pos) == id);       	\
      return id;							\
  }
#define INSERT_FULL_2(t1,a1,t2,a2)					\
  ...
  {									\
      ...
      clearDummy ();							\
									\
      unsigned pos = itemPosition (id);					\
      VERIFY (m_menu->insertItem (a1, "full", receiver, member, accel, id, pos) == id);\
      return id;							\
  }
...
IgQtObjectMenu::IgQtObjectMenu (QMenu *menu)
    : m_menu (menu),
      m_dummy (-1)
{
...
IgQtObjectMenu::subMenuPosition (int id, bool exact) const
{
	...
	ASSERT (! item->menu () || j < m_submenus.size ());
	ASSERT (! item->menu () || m_submenus [j]->m_menu == item->menu ());
...
INSERT_SUB_2	(QIcon, icon, QString, text)

INSERT_SUB_1	(QPixmap, pixmap)
INSERT_SUB_2	(QIcon, icon, QPixmap, pixmap)

INSERT_SIMPLE_1 (QString, text)
INSERT_FULL_1   (QString, text)
INSERT_SIMPLE_2 (QIcon, icon, QString, text)
INSERT_FULL_2   (QIcon, icon, QString, text)

INSERT_SIMPLE_1 (QPixmap, pixmap)
INSERT_FULL_1   (QPixmap, pixmap)
INSERT_SIMPLE_2 (QIcon, icon, QPixmap, pixmap)
INSERT_FULL_2   (QIcon, icon, QPixmap, pixmap)
...
SHADOW_GET   (iconSet,       QIcon )

SHADOW_GET   (text,          QString)
SHADOW_SET_1 (changeItem,    QString, text)
SHADOW_SET_2 (changeItem,    QIcon, icon, QString, text)

SHADOW_GET   (pixmap,        QPixmap )
SHADOW_SET_1 (changeItem,    QPixmap, pixmap)
// FIXME Qt 4 SHADOW_SET_2 (changeItem,    QIcon, icon, QString, pixmap)
...
IgQtObjectMenu::setDummy (void)
{
    ...
    // not.
    if (m_menu->count () == 0 && ! dynamic_cast<QMenu *> (m_menu->parent ()))
    {
	ASSERT (m_dummy == -1);
