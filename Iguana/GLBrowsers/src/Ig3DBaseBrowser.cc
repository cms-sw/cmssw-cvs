...
Ig3DBaseBrowser::initialise (IgState *state, IgSite *site)
{
    ...
    QMdiArea *workspace = dynamic_cast<IgPage *>(site)->workspace ();
    QWidget *w = new QWidget(workspace,Qt::Widget);
    w->setMinimumSize (200, 150);
    m_mdiWindow = new QMdiSubWindow;
    m_mdiWindow->setWidget (w);
    m_mdiWindow->setOption (QMdiSubWindow::RubberBandResize);
    workspace->addSubWindow (m_mdiWindow);
...
void
Ig3DBaseBrowser::changeWindowState (Qt::WindowStates oldState, Qt::WindowStates newState)
{
    this->windowStateChanged (oldState, newState);
}
...
Ig3DBaseBrowser::initWidget (IgSite *site)
{
    ...
    QObject::connect (m_mdiWindow, SIGNAL(windowStateChanged (Qt::WindowStates, Qt::WindowStates)),
...
Ig3DBaseBrowser::print (void)
{
    ...
    QFileDialog	dialog (getShellWidget (), tr("Print To File"));
...
void
Ig3DBaseBrowser::autoPrint (const std::string text)
{
                           ...
                           QWidget* parent /* = 0 */, const char* file /*= 0*/)
{
	...
	while (tryagain)
	{
		...
		else if (!lat::Filename(dialog.directory ().path().latin1()).isWritable ())
		{
		    int button = QMessageBox::warning (parent, "Access denied",
						       "You do not have permissions to write in \""+dialog.directory ().path()+"\" directory.\n"
						       "Do you want to select some other file?",
						       "Yes", "No");
		    if (button == 1)
			return false;
...
{
    // note:
    // SoDB::read (&file, node) [sometimes] has a buggy behaviour when 
    // loading *.iv files. Some parts of the scene graph might get copied 
    // because of a still unknown reason. For example when tried to save a
    // "grid planes" graph, the grid plane would get a "shadow" grid.
