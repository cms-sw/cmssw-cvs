...
#include "Iguana/GLBrowsers/interface/Ig3DMFloatControl.h"
...
#include "Iguana/GLBrowsers/interface/Ig3DMUShortControl.h"
...
#define SMRANGE_FIELD_DELETE_OPERATIONS(CONTROL_TYPE)				\
	...
	{SbName  fieldName;							\
	CONTROL_TYPE* cfield = 	dynamic_cast<CONTROL_TYPE *> (m_fields [field]);\
	QObject::disconnect (cfield, SIGNAL (settingsChanged (SoField*)), \
...
#define RANGE_FIELD_SET_LIMITS							\
	...
	{QString s(m_fieldLimitsMap[key]); cfield->setSettingString (s);}	\
        QObject::connect (cfield, SIGNAL (settingsChanged (SoField*)),	\
			  this,   SLOT (saveSettings (SoField*)));
...
void
Ig3DNodeCategory::focusInEvent(QFocusEvent */*event*/)
{
  focusIn();
}
...
Ig3DNodeCategory::focusIn (void)
{
        ...
        itr != m_nodeMap.end (); itr++)
    {
      ...
      {
	QTreeWidgetItem *top = itr->first->parent ();
	while (top)
	{
	  top->setExpanded(true);
	  top = top->parent ();
	}
	m_nodeTree->setCurrentItem(itr->first);
	break;
      }
    }
}
...
void Ig3DNodeCategory::saveNode (void)
{
    ASSERT (m_source);
    m_source->saveNode(m_currentNode,"Save Selected Node As...",panel ());
}

//////////////////////////////////////////////////////////////////////

void
Ig3DNodeCategory::manipulateItem (QTreeWidgetItem *item)
{
  if(!item) return;
    if (m_nodeMap.find (item) != m_nodeMap.end ())
    {
      ...
      QPushButton *saveme = new QPushButton ("Save Node", area);
      ...
      QObject::connect (saveme,		SIGNAL (clicked ()),
			this, 		SLOT   (saveNode ()));
...
{      
    ...
    if (!m_nodeTree)
    {
      m_nodeTree = new QTreeWidget (leftArea ());
      m_nodeTree->setWindowTitle("Node Manipulator List");
      m_nodeTree->setHeaderLabels(QStringList("Node Type/Name"));
      m_nodeTree->setSelectionMode(QAbstractItemView::ExtendedSelection);
      leftLayout ()->addWidget (m_nodeTree);
      QObject::connect (m_nodeTree, SIGNAL ( itemActivated (QTreeWidgetItem *, int)),
			this,       SLOT   ( manipulateItem (QTreeWidgetItem *)));
    }
    m_nodeTree->clear ();
    m_nodeMap.clear ();
    m_nodeTree->show ();
    
    QTreeWidgetItem *topItem = new QTreeWidgetItem(QStringList("Nodes List"));
    m_nodeTree->addTopLevelItem(topItem);
    //topItem->setSelectable (false);
    addNode (node, topItem);
    topItem->setExpanded(true);
    if(topItem->childCount() == 0)
      manipulate(0);
    else if (topItem->child(1))
      m_nodeTree->setCurrentItem(topItem->child(1));
}

bool
Ig3DNodeCategory::addNode (SoNode *node, QTreeWidgetItem *parent /* = 0 */ )
{
    std::string name("");
    ...
    if(parent == 0)
    {      
      item = new QTreeWidgetItem(QStringList(name.c_str ()));
...
Ig3DNodeCategory::clearCategory (void)
{
    ...
    fieldControlMap::iterator item;
    for(item = m_fields.begin (); item != m_fields.end (); item++)
      QObject::disconnect (item->second, SIGNAL (settingsChanged (SoField*)),
			   this,         SLOT (saveSettings (SoField*)));
    m_fields.clear ();
}
