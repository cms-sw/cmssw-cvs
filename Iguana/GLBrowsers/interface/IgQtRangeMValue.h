#ifndef IGUANA_GL_BROWSERS_IG_QT_RANGE_MVALUE_H
# define IGUANA_GL_BROWSERS_IG_QT_RANGE_MVALUE_H
...
template<class T>
class IGUANA_GL_BROWSERS_API IgQtRangeMValue
{
public:
    ...
    IgQtRangeMValue (unsigned int mySize,
                     const T* values,
                     T min = 0,
		     T max = SHRT_MAX);
    ...
    unsigned int	mySize (void) const
    { return m_value.size(); }
    
    virtual void	myReSize (unsigned int mySize, T appendedValues = 0)
    {
      ASSERT (mySize);
      if (mySize == m_value.size ()) return;
      m_value.resize(mySize, appendedValues);
      m_change.resize(mySize, 0);
    }
    ...
    virtual void	setValue (unsigned int mySize, const T* values)
    {
...
protected:
    ...
    virtual void   	forceValue (unsigned int mySize, const T* values)
    {
      for (unsigned int i = 0; i < mySize; i++)
    ...
    {
      unsigned int mySize = values.size ();
      for (unsigned int i = 0; i < mySize; i++)
        forceValue (i, values[i]);
    }
    ...
    {
      unsigned int mySize = m_change.size ();
      for (unsigned int i = 0; i < mySize; i++)
        m_change [i] = delta;
    ...
    virtual void   	forceValueChanged (unsigned int mySize, const T* delta)
    {
      m_valueChangeFlag = false;
      for (unsigned int i = 0; i < mySize; i++)
    ...
    {
      unsigned int mySize = delta.size ();
      m_valueChangeFlag = false;
      for (unsigned int i = 0; i < mySize; i++)
        forceValueChanged (i, delta[i]);
    }
...
template<class T>
IgQtRangeMValue<T>::IgQtRangeMValue (unsigned int mySize,
				     const T* values,
                                     T min /* = 0 */,
				     T max /* = SHRT_MAX */)
    : m_value (mySize, 0),
...
#endif // IGUANA_GL_BROWSERS_IG_QT_RANGE_MVALUE_H
