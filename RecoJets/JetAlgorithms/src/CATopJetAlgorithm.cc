							...
							edm::EventSetup const & c )  
{
	...
	for ( ; jetIt != centralJetsEnd; ++jetIt ) {
		...
		bool hardBreak1 = decomposeJet(localJet,clusterSeq,cell_particles,towerGeometry,ptHard,nCellMin,deltarcut,ja,jb,leftovers1);
		...
		if (hardBreak1)  hardBreak2a = decomposeJet(ja,clusterSeq,cell_particles,towerGeometry,ptHard,nCellMin,deltarcut,jaa,jab,leftovers2a);
		...
		if (hardBreak1)  hardBreak2b = decomposeJet(jb,clusterSeq,cell_particles,towerGeometry,ptHard,nCellMin,deltarcut,jba,jbb,leftovers2b);
...
bool CATopJetAlgorithm::decomposeJet(const fastjet::PseudoJet & theJet, 
	...
	while (1) {                                                      // watch out for infinite loop!
		...
		if ( useAdjacency_==3 &&  adjacentCells(ja,jb,cell_particles,fTowerGeometry,theClusterSequence,nCellMin) ){                  
